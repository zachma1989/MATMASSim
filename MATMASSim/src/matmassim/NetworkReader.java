...
public class NetworkReader extends MatsimXmlParser {
	...
	private final static Logger log = Logger.getLogger(NetworkReader.class);

	public NetworkReader() {
		super();
		this.scenario = null;
		this.network = (Network_MASS) scenario.getNetwork();
	}
	...
	private void startNode(final Attributes atts) {
		...
		Node node = (Node) this.network.getFactory().createNode(this.scenario.createId(atts.getValue("id")), new CoordImpl(atts.getValue("x"), atts.getValue("y")));
		this.network.addNode(node);
		if (node instanceof Element_MASS) {
			((Element_MASS) node).setType(atts.getValue("type"));
			if (atts.getValue("origid") != null) {
				((Element_MASS) node).setOrigId(atts.getValue("origid"));
			}
		}
	}
	...
	private void startLink(final Attributes atts) {
		...
		Link l = (Link) this.network.getFactory().createLink(this.scenario.createId(atts.getValue("id")), fromNode, toNode);
